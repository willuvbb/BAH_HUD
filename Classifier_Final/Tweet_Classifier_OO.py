...
class Classifier(object):
    ...
    def save_tweets_to_csv(self):
        ...
        # Code imported from my "GetStateCounts.py". I shouldn't have to save and then reload, but I didn't want to
        # rename the variables. Sorry.
    ...
    def run(self):
        ...
        '''The save_to_elasticsearch() method can be called, but look first in the HUD_WebApp_Final folder. This folder
        # has copies of the files exported by this code. In that folder there is a file called setup_elastic.py which 
         makes an es index for both the twitter data and the state counts. If you combined the classifier folder and 
         the WebApp folder, it'd probably be best to update the save_to_elasticsearch() method here based on what is 
         currently saved/written in setupelastic.py. Then you wouldn't have to run that before doing the webapp.'''
