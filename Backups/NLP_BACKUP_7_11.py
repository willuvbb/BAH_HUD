...
# dataset = dataset.drop(dataset[any(phrase in dataset['tweet_text'] for phrase in keywords_to_remove)].index)
# dataset = dataset.drop(dataset[any([i.find(phrase)!= -1 for phrase in keywords_to_remove]) for i in dataset["tweet_text"]].index)
# dataset = dataset.drop([any(i.find(phrase)!= -1 for phrase in keywords_to_remove) for i in dataset["tweet_text"]])
# dataset = dataset.drop([any([i.find(phrase) != -1 for phrase in keywords_to_remove]) for i in dataset["tweet_text"]])
...
# [any([i.find(phrase)!= -1 for phrase in keywords_to_remove])for i in dataset["tweet_text"]]
# dataset = dataset.drop(dataset['australia' in dataset['tweet_text']].index)
# Take out rows that have nan in the primary category column
# ~~~~~~~~~~~~~
...
# Define the model training and validation function
def train_model_NEW(classifier, classifier_title, feature_vector_train, feature_vector_test, feature_title,
                    y_train, y_test, model_score_df):
    # fit the training dataset on the classifier
    classifier.fit(feature_vector_train, y_train)
    ...
    # Store the accuracy score in the model_score dataframe
    # model_score_df.loc[classifier_title].at[feature_title] = my_accuracy_score
    model_score_df.at[classifier_title, feature_title] = my_accuracy_score
    ...
    return my_accuracy_score, conf_mat, kappa_score, model_score_df

# Define the model training and validation function
def train_model_NEW_dict(classifier, classifier_title, feature_vector_train, feature_vector_test, feature_title,
...
def compare_model_scores_OLD(model_score_df, X_train_count, y_train, X_test_count, y_test):
    '''------------Naive Bayes------------'''
...
for classy in classifier_list:
    for feat in feature_list:
        scores_dict = train_model_NEW_dict(*classy, *feat, y_train, y_test, scores_dict)
        # print(scores_dict)
        # print(" ")
        # EXAMPLE:
        ...
        # print(model_score_df.to_string())
...
for i in features_multiplier_arr:
    ...
    if do_compare_models:
        scored_df = compare_model_scores_OLD(model_score_df, X_train_count, y_train, X_test_count, y_test)
...
sns.heatmap(scores_dict['Naive Bayes Classifier']['CNT Vec']['conf_mat'], annot=True, fmt='d',
