...
X_train, X_test, y_train, y_test = train_test_split(no_token_corpus, tweets['secondary_category_id'], test_size = 0.30, random_state=0)
...
tweets.groupby('Secondary').count().plot.bar(ylim=0)
plt.show()
...
sns.heatmap(scores_dict['Naive Bayes Classifier']['CNT Vec']['conf_mat'], annot=True, fmt='d',
            xticklabels=secondary_category_id_df.Secondary.values, yticklabels=secondary_category_id_df.Secondary.values)
plt.ylabel('Actual')
